...
SELECT
    ...
    CAST(
        COUNT(*) * 1.0 / COUNT(DISTINCT order_id) AS DECIMAL(5, 2)
    ) as media_items_per_ordine
FROM order_items;
...
SELECT
    ...
    CAST(
        AVG(oi.quantity) AS DECIMAL(10, 2)
    ) as quantita_media_per_ordine,
    CAST(
        SUM(oi.quantity * oi.price) AS DECIMAL(10, 2)
    ) as fatturato_totale
FROM order_items oi
...
SELECT
    ...
    CAST(
        AVG(oi.price) AS DECIMAL(10, 2)
    ) as prezzo_medio_vendita
FROM order_items oi
    INNER JOIN products p ON oi.product_id = p.id
GROUP BY
    p.id,
...
SELECT
    ...
    CAST(
        AVG(oi.price) - p.price AS DECIMAL(10, 2)
    ) as differenza_media,
    CAST(
        (AVG(oi.price) - p.price) / p.price * 100 AS DECIMAL(5, 2)
...
GROUP BY
    ...
    p.price
HAVING
    SUM(oi.quantity) > 5 -- Solo prodotti con vendite significative
...
FROM
    ...
    INNER JOIN orders o ON oi.order_id = o.id
WHERE
    o.created_at >= DATEADD(day, -30, GETDATE())
GROUP BY
    p.id,
    p.code,
    p.name,
...
SELECT 'Items con quantità zero o negativa' as controllo, COUNT(*) as count
FROM order_items
WHERE
    quantity <= 0
UNION ALL
SELECT 'Items con prezzo zero o negativo' as controllo, COUNT(*) as count
FROM order_items
WHERE
    price <= 0
UNION ALL
SELECT 'Items senza ordine collegato' as controllo, COUNT(*) as count
FROM order_items oi
    LEFT JOIN orders o ON oi.order_id = o.id
WHERE
    o.id IS NULL
UNION ALL
SELECT 'Items senza prodotto collegato' as controllo, COUNT(*) as count
FROM order_items oi
...
SELECT 'Totale items venduti' as metrica, CAST(COUNT(*) AS VARCHAR(20)) as valore
FROM order_items
UNION ALL
SELECT 'Quantità totale venduta' as metrica, CAST(SUM(quantity) AS VARCHAR(20)) as valore
FROM order_items
UNION ALL
SELECT 'Fatturato totale' as metrica, CAST(
        CAST(
            SUM(quantity * price) AS DECIMAL(10, 2)
        ) AS VARCHAR(20)
    ) as valore
FROM order_items
UNION ALL
SELECT 'Ordini con items' as metrica, CAST(
        COUNT(DISTINCT order_id) AS VARCHAR(20)
    ) as valore
FROM order_items
UNION ALL
SELECT 'Prodotti venduti' as metrica, CAST(
        COUNT(DISTINCT product_id) AS VARCHAR(20)
    ) as valore
FROM order_items
UNION ALL
SELECT 'Prezzo medio item' as metrica, CAST(
        CAST(AVG(price) AS DECIMAL(10, 2)) AS VARCHAR(20)
    ) as valore
FROM order_items;

-- 🏆 Clienti per valore degli acquisti (tramite order_items)
SELECT
    TOP 10 c.name as nome_cliente,
    c.email as email_cliente,
    COUNT(DISTINCT oi.order_id) as numero_ordini,
    SUM(oi.quantity) as quantita_totale_acquistata,
    CAST(
        SUM(oi.quantity * oi.price) AS DECIMAL(10, 2)
    ) as valore_totale_acquisti,
    CAST(
        AVG(oi.quantity * oi.price) AS DECIMAL(10, 2)
    ) as valore_medio_per_item
FROM
    order_items oi
    INNER JOIN orders o ON oi.order_id = o.id
...
SELECT
    ...
    CAST(
        SUM(oi.quantity * oi.price) AS DECIMAL(10, 2)
    ) as fatturato_settimanale
FROM order_items oi
    INNER JOIN orders o ON oi.order_id = o.id
WHERE
    o.created_at >= DATEADD(week, -12, GETDATE())
GROUP BY
    YEAR(o.created_at),
