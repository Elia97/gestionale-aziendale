...
/**
 * Componente per visualizzare i dettagli di un magazzino.
 * Mostra informazioni come nome, indirizzo, statistiche e giacenze dei prodotti.
 * Gestisce la visualizzazione mobile e desktop in base alla larghezza dello schermo.
 * @param isViewModalOpen - Indica se la modale di visualizzazione è aperta.
 * @param setIsViewModalOpen - Funzione per chiudere la modale di visualizzazione.
 * @param selectedWarehouse - Magazzino selezionato da visualizzare.
 * @param getWarehouseStats - Funzione per ottenere le statistiche del magazzino.
 */
              ...
              <CardContent className="p-3">
                <div className="space-y-2">
                  ...
                  <div className="flex justify-between items-center text-sm">
                    ...
                    <Badge
                      variant={
                        stock.quantity <= 10 ? "destructive" : "secondary"
                      }
                    >
                      {stock.quantity}
                    </Badge>
                  ...
                  <div className="flex justify-between items-center text-sm font-medium border-t pt-2">
                    ...
                    <span>
                      {formatCurrency(
                        stock.quantity * Number(stock.product.price)
                      )}
                    </span>
                  </div>
                </div>
              ...
              selectedWarehouse.stocks.map((stock) => (
                <TableRow key={stock.id}>
                  ...
                  <TableCell className="text-right">
                    {formatCurrency(Number(stock.product.price))}
                  </TableCell>
    ...
    <Dialog open={isViewModalOpen} onOpenChange={setIsViewModalOpen}>
      <DialogContent className="sm:max-w-[700px] max-h-[90vh] flex flex-col">
        <DialogHeader className="flex-shrink-0">
          <DialogTitle>
            Dettaglio Magazzino: {selectedWarehouse?.name}
          </DialogTitle>
          <DialogDescription>
            Visualizza le informazioni complete del magazzino e le giacenze.
          </DialogDescription>
        </DialogHeader>
        {selectedWarehouse && (
          <div className="space-y-4 overflow-y-auto flex-1 pr-2 -mr-2">
            ...
            <div>
              ...
              <div className="mt-2">{renderStockItems()}</div>
            </div>
          </div>
        )}
